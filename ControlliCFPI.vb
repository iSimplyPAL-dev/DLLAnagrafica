...
Namespace DLL
    Public Class ControlliCFPI
        Private Log As ILog = LogManager.GetLogger(GetType(ControlliCFPI))
        ...
        'WorkFlow
        '===============================================================================
        'Public Function WF_Calcolo_Codice_Fiscale(ByVal Cognome As String, ByVal Nome As String, ByVal DataNascita As String, ByVal sesso As String, ByVal Comune As String, ByRef message As String) As String
        '    Dim mesi As String
        ...
        'End Function
        'Public Function WF_Luogo_Nascita_da_CodFiscale(ByVal CodFiscale As String, ByVal Msg As Boolean, ByRef message As String,
        ...
        '                    Luogo_Nascita_da_CodFiscale = AnagUtility.GetParametro(DrComune.Item("COMUNE"))
        '                    Provincia = AnagUtility.GetParametro(DrComune.Item("PV"))
        '                Else
        '                    If Msg = True Then
        ...
        Public Function Calcolo_Codice_Fiscale(ByVal Cognome As String, ByVal Nome As String, ByVal DataNascita As String, ByVal sesso As String, ByVal Comune As String, ByRef message As String, TypeDB As String, ByVal StringConnection As String) As String
            Dim mesi As String
            Dim Vocali As String
            Dim Consonanti As String
            Dim Numeri As String
            ...
            Try
                ...
                'SQLString = "SELECT * FROM V_ENTI"
                'SQLString = SQLString + " WHERE (((COMUNE+PV)= '" & Replace(Comune, "'", "''") & "'));"
                ''DataRBelfiore = objDBAccess.GetPrivateDataReader(SQLString)
                'Log.Debug(SQLString)
                'cmdMyCommand.CommandType = CommandType.Text
                'cmdMyCommand.CommandText = SQLString
                'DataRBelfiore = cmdMyCommand.ExecuteReader()
                'If DataRBelfiore.Read = True Then
                '    CodiceFiscaleComune = DataRBelfiore.Item("IDENTIFICATIVO")    'objrecordset("COD_BELFIORE")
                'Else
                '    message = "Codice Fiscale Sconosciuto"
                ...
                'End If
                'DataRBelfiore.Close()
                Dim DBAccess As New getDBobject(TypeDB, StringConnection)
                Try
                    dsBelfiore = DBAccess.GetDataSet("prc_GetComuni", New ArrayParam("@COMUNEPV", Comune) _
                                                                    , New ArrayParam("@BELFIORE", String.Empty))

                    For Each myRow As DataRow In dsBelfiore.Tables(0).Rows
                        CodiceFiscaleComune = myRow("identificativo")
                    Next
                Catch ex As Exception
                    Log.Debug("AnagraficaDLL::ControlliCFPI::calcolo_codice_fiscale::si è verificato il seguente errore::", ex)
                Finally
                    dsBelfiore.Dispose()
                End Try
                '------------------------------------------------------------------------------------------------------------------------------
        ...
        Public Function Luogo_Nascita_da_CodFiscale(ByVal CodFiscale As String, ByVal Msg As Boolean, ByRef message As String, ByRef Identificativo As String, ByRef Provincia As String, TypeDB As String, ByVal StringConnection As String) As String
            Dim CodCom As String
            Luogo_Nascita_da_CodFiscale = ""
            'CALCOLO IL LUOGO DI NASCITA.
            'SAPENDO CHE LA LETTERA CHE SI TROVA IN POSIZIONE 12 E LE TRE CIFRE SUCCESSIVE CORRISPONDO AL
            'CODICE BELFIORE DEL COMUNE DI NASCITA, OTTENGO IL LUOGO
            ...
            Else
                ...
                Else
                    ...
                    Try
                        ...
                        'DrComune = cmdMyCommand.ExecuteReader()
                        'If DrComune.Read = True Then
                        '    Luogo_Nascita_da_CodFiscale = AnagUtility.GetParametro(DrComune.Item("COMUNE"))
                        '    Provincia = AnagUtility.GetParametro(DrComune.Item("PV"))
                        'Else
                        ...
                        Dim DBAccess As New getDBobject(TypeDB, StringConnection)
                        Try
                            dsComune = DBAccess.GetDataSet("prc_GetComuni", New ArrayParam("@COMUNEPV", String.Empty) _
                                                           , New ArrayParam("@BELFIORE", CodCom))
                            For Each myRow As DataRow In dsComune.Tables(0).Rows
                                Luogo_Nascita_da_CodFiscale = AnagUtility.GetParametro(myRow("COMUNE"))
                                Provincia = AnagUtility.GetParametro(myRow("PV"))
                            Next
                        Catch ex As Exception
                            Log.Debug("AnagraficaDLL::ControlliCFPI::luogonascita_da_codice_fiscale::si è verificato il seguente errore::", ex)
                        Finally
                            dsComune.Dispose()
                        End Try
                    Catch ex As Exception
                        ...
                        '    '********************Gestione Anagrafiche massive****************************
                    End Try
                    Return Luogo_Nascita_da_CodFiscale
                End If
            End If
        End Function
